...
当前的瓶颈已转移至：（1）\textbf{setup 41ms（32\%）}——每层重复的configure + load\_weights + load\_bias，可通过layer-wise预加载与CSR批量写入优化，目标降至约10 ms；（2）\textbf{load\_x 23ms（18\%）}——51次MVM调用中每次的stream sink arm + DMA transfer开销，可通过批量输入预打包减少DMA启动次数；（3）\textbf{im2col 21ms（16\%）}——纯Python循环构建col\_matrix，可通过NumPy stride\_tricks或Numba JIT加速至约5 ms。三者合计可从约85 ms优化至约25 ms，预计端到端可降至约60--80 ms/img（12--16 fps）。以下§\ref{sec:sw_optimization}--\ref{sec:phase_b}依次记录了在这些瓶颈方向上的实际优化过程与最终成果。
...
\begin{table}[H]
    ...
    \begin{tabular}{lccccc}
        ...
        \textbf{Phase B 乒乓} & \textbf{29.2} & \textbf{34.3} & \textbf{1.27×} & \textbf{57.9×} & IBUF/OBUF双缓冲DMA重叠 \\
        \bottomrule
    \end{tabular}
\end{table}

从MMIO到Phase B，累计端到端加速比达到\textbf{57.9倍}，吞吐率从0.59 fps提升至34.3 fps。全程分类准确率保持99.5\%不变，所有阶段输出与Python golden model bit-exact一致。
...
（6）\textbf{AXI4-Stream DMA数据通路重构与全链路优化}：通过九阶段递进优化将LeNet-5端到端延迟从1,690.5 ms/img降至29.2 ms/img（\textbf{57.9×端到端加速}，34.3 fps），累计加速比高于学术界同类FPGA CIM工作一到两个数量级：
\begin{itemize}
...
（6）\textbf{AXI4-Stream DMA数据通路优化}：本工作已完成从MMIO到Phase B乒乓的九阶段全链路优化（§\ref{sec:c3_dma}--\ref{sec:phase_b}），将LeNet-5端到端延迟从1,690.5 ms/img降至29.2 ms/img（57.9×加速）。当前剩余瓶颈为$\sim$25.4 ms的MVM乒乓wall time（max(DMA, compute)路径，受限于DMA带宽与TILE\_SPLIT\_FACTOR=4下的硬件compute时间）以及$\sim$3.7 ms的不可重叠Python开销（im2col 1.22ms + pool 1.24ms + pack 0.66ms + setup 0.59ms）。进一步优化方向包括：（a）将MaxPool硬件化，在CIM核心的ST\_STORE后增加pooling FSM，消除软件pool延迟并减少一次DMA round-trip；（b）Layer Fusion——将Conv→Pool→Conv的中间结果保留在OBUF/IBUF中不写回DDR，省去层间数据搬运；（c）将PYNQ-Z2上的成果移植到KV260平台，利用更大的BRAM（360+）和更快的fabric提升$k_{\text{pack}}$并降低TILE\_SPLIT\_FACTOR。当前29.2 ms/img（34.3 fps）已远超原预测的60--80 ms/img目标。
