...
这一结论直接指导了后续的优化方向选择。原先计划的"软件侧权重常驻缓存"方案因weight SRAM跨层共享的硬件约束被证明不可行（见§\ref{sec:future_work}的讨论），而真正根治此瓶颈的做法是将AXI4-Lite替换为AXI4-Stream DMA引擎。基于此profiler分析，本工作实施了C3（MM2S DMA写入，§\ref{sec:c3_dma}）与P0（S2MM DMA读回，§\ref{sec:p0_s2mm}）两阶段AXI4-Stream DMA数据通路重构——C3将weight/input/bias的搬运从逐字MMIO改为DMA批量传输（端到端503.7 ms/img，3.4×加速），P0将read\_output从257 ms串行MMIO改为单次DMA S2MM传输（降至19.65 ms，13.1×加速），最终实现128.4 ms/img（13.2× vs MMIO baseline）。优化后的逐项延迟分解见表\ref{tab:p0_latency}。
...
\subsection{C1 流水线分离优化：80 MHz时序收敛}
\label{sec:c1_pipeline}

55 MHz变体虽能时序收敛，但硬件频率下降导致理论MAC吞吐率降低了约9\%（555$\to$505 GMAC/s）。为在更高频率下获得时序收敛，本工作实施了C1流水线分离优化，将后量化路径（原ST\_SHIFT\_CLAMP状态）中的barrel shifter与rounding加法器拆分为两级流水。

\subsubsection{关键路径分析}

§\ref{sec:55mhz}的timing报告表明，60 MHz构建的关键路径并非CIM Tile的MAC链（SPLIT=4已将MAC路径控制在约5 ns），而是位于后量化阶段。具体而言，原实现在单周期内完成：

\begin{equation}
    \text{shifted} = \left\lfloor \frac{\text{prod\_r} + 2^{\text{shift}-1}}{2^{\text{shift}}} \right\rfloor
\end{equation}

其中$\text{prod\_r}$为INT64中间乘积，$\text{shift}\in[0,31]$为可变移位量。Vivado综合报告显示，该组合路径包含31个逻辑层次（22个CARRY4进位链），数据路径延迟达11.5 ns，远超10 ns目标周期。

\subsubsection{流水线分离方案}

优化方案将后量化路径拆分为两个独立状态（如图\ref{fig:c1_pipeline}所示）：

\begin{itemize}
    \item \textbf{ST\_SHIFT（新）}：仅执行rounding加法 $\text{round} = \text{prod\_r} + (1 \ll (\text{shift} - 1))$，结果锁存至$\text{pre\_shift\_r}$寄存器；
    \item \textbf{ST\_CLAMP（新）}：在注册的$\text{pre\_shift\_r}$上执行barrel shift + clamp至INT8范围，结果直接送输出缓冲。
\end{itemize}

由于$\text{prod\_r}\to\text{pre\_shift\_r}$和$\text{pre\_shift\_r}\to\text{requant\_r}$两条路径均从寄存器开始，关键路径被限制在单级barrel shifter的逻辑深度内（约10 ns），不再包含rounding加法器的22个CARRY4进位链。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/m6.png}
    \caption{C1流水线分离：ST\_SHIFT（rounding）→ ST\_CLAMP（barrel shift + clamp）}
    \label{fig:c1_pipeline}
\end{figure}

\subsubsection{综合结果}

表\ref{tab:c1_synth}汇总了C1优化后的多频率构建结果。80 MHz构建在C1优化后实现时序收敛（WNS=+0.059 ns），成为当前SPLIT=4配置下的最高稳定频率。

\begin{table}[H]
    \centering
    \caption{C1流水线分离优化后多频率构建结果（Vivado 2024.2，PYNQ-Z2，PAR\_OB=1）}
    \label{tab:c1_synth}
    \begin{tabular}{lcccccc}
        \toprule
        频率 & 周期(ns) & WNS(ns) & WHS(ns) & 时序状态 & 最高频率达成 \\
        \midrule
        55 MHz & 18.182 & +2.000 & +0.693 & CLEAN & -- \\
        60 MHz & 16.667 & $-0.086$ & +0.050 & 3 failing EP & -- \\
        70 MHz & 14.286 & +0.021 & +0.050 & CLEAN & -- \\
        80 MHz & 12.500 & +0.059 & +0.050 & \textbf{CLEAN} & \checkmark \\
        100 MHz & 10.000 & $-1.512$ & +0.010 & FAIL (1948 EP) & -- \\
        125 MHz & 8.000 & $-5.888$ & +0.026 & FAIL & -- \\
        \bottomrule
    \end{tabular}
\end{table}

\subsubsection{80 MHz板级验证}

将80 MHz bitstream部署至PYNQ-Z2，使用与前述测试相同的200张MNIST图片进行LeNet-5端到端验证，结果如表\ref{tab:c1_benchmark}所示。

\begin{table}[H]
    \centering
    \caption{C1优化后多频率版本性能对比（LeNet-5，200张MNIST，PYNQ-Z2）}
    \label{tab:c1_benchmark}
    \begin{tabular}{lcccc}
        \toprule
        指标 & 55 MHz & 60 MHz & 70 MHz & 80 MHz \\
        \midrule
        时序收敛 & WNS=+2.0 ns & WNS=$-0.086$ ns & WNS=+0.021 ns & WNS=+0.059 ns \\
        总wall time（200张）& 343.9 s & 340.1 s & 325.5 s & 338.1 s \\
        单张平均延迟 & 1,719.7 ms & 1,700.4 ms & 1,627.7 ms & 1,690.7 ms \\
        LeNet-5分类准确率 & 99.5\% & 99.5\% & 99.5\% & 99.5\% \\
        Bit-exact一致率 & 100\% & 100\% & 100\% & 100\% \\
        \bottomrule
    \end{tabular}
\end{table}

80 MHz版本在功能正确性（准确率99.5\%，bit-exact 100\%）上与60/55 MHz完全一致，端到端wall time为338.1 s，与60 MHz（340.1 s）基本持平。与55 MHz相比，80 MHz在相同功能正确性的前提下提供了约45\%的理论频率提升（55$\to$80 MHz）。

\textbf{频率与端到端性能的解耦}：四个频率版本（55/60/70/80 MHz）的wall time标准差为8.0 s（变异系数2.4\%），均在测量误差范围内。70 MHz测得325.5 s（相对平均值-3.4\%）是统计异常值，而55/60/80 MHz均在平均值$\pm$2\%范围内。更重要的是，§\ref{sec:profiler}的profiler数据表明硬件compute仅占端到端延迟的0.24\%（约4 ms），即使频率从55 MHz提升至80 MHz（+45\%），硬件compute时间从约4.4 ms降至约3.0 ms，对总延迟1700 ms的影响仅为0.08\%（约1.4 ms），完全淹没在MMIO传输（约730 ms，43\%）与Python开销（约962 ms，57\%）的波动中。

综合来看，端到端性能完全由AXI4-Lite MMIO开销主导，硬件频率提升对整体性能的影响可忽略不计（$<$0.1\%）。C1优化的价值在于验证了RTL在80 MHz下时序收敛的能力，为未来DMA重构后高频场景提供了时序余量——届时硬件compute占比将从0.24\%提升至约60\%（预测端到端约6 ms/img），频率提升才会转化为实际性能增益。
...
表\ref{tab:c3_benchmark}对比了C3重构前后LeNet-5在PYNQ-Z2上的200张MNIST端到端实测性能。表中"重构前（MMIO）"为同一C3 bitstream上运行legacy MMIO软件路径（\texttt{use\_dma=False}）的实测数据，"重构后（DMA）"为DMA软件路径（\texttt{use\_dma=True}）的实测数据（\texttt{benchmark\_e2e\_60mhz\_dma.csv}）。
...
\begin{table}[H]
    ...
    \begin{tabular}{lccc}
        ...
        \midrule
        总wall time（200张） & 338.1 s & 100.7 s & \textbf{3.35×} \\
        单张平均延迟 & 1,690.5 ms & 503.7 ms & \textbf{3.35×} \\
        分类准确率（bit-exact） & 99.5\%（199/200） & 99.5\%（199/200） & 一致 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\subsubsection{DMA路径瓶颈分析与进一步优化方向}
\label{sec:c3_bottleneck}

DMA路径实测503.7 ms/img，相比MMIO的1690.5 ms加速3.35×，但距理论预测的7.6 ms/img仍有约66×差距。通过对DMA路径的profiler数据分析，当前瓶颈已从"数据传输"转移至以下两个方面：

\textbf{（1）Python侧im2col与权重打包开销}：Conv1的setup时间（约299 ms）主要来自\texttt{weight\_to\_chunks()}对打包权重矩阵的Python循环构造，以及im2col的逐像素展开。这部分与DMA无关，是纯Python计算开销。优化方向：将im2col与权重打包用NumPy向量化或C扩展实现，预计可将setup从300 ms降至$<$10 ms。

\textbf{（2）PYNQ DMA单次传输上限（16 KB）导致的分块开销}：LeNet-5 Conv1权重约170 KB，需分42次DMA调用，每次调用含一次\texttt{mmio.write(CSR\_STREAM\_LEN)}触发cfg\_start、一次\texttt{dma.sendchannel.transfer()}、一次\texttt{dma.sendchannel.wait()}，Python侧每次约50--100 $\mu$s，42次合计约2--4 ms，相对于传输本身可忽略，但与理论值的差距说明主要瓶颈仍在Python开销而非DMA带宽。

\textbf{（3）关键路径与125 MHz目标}：表\ref{tab:c1_synth}显示C1优化后80 MHz时序收敛（WNS=+0.059 ns），100 MHz仍有1948个failing endpoint（WNS=$-1.512$ ns），125 MHz的WNS=$-5.888$ ns。当前关键路径在C1优化后已从后量化barrel shifter转移至其他路径，需进一步分析。达到125 MHz的主要障碍预计在以下两处：

\begin{itemize}
    \item \textbf{MAC累加链}：\texttt{psum\_accum}中的INT32加法树在PAR\_OB=4时跨越多个DSP48E1级联，进位链延迟约8--9 ns，在125 MHz（8 ns周期）下无裕量。优化方案：将累加拆为两级流水（先局部累加再全局累加），或将PAR\_OB降至2以缩短加法树深度。
    \item \textbf{重量化乘法器}：\texttt{requant\_mult}为32×32位乘法，Vivado映射到DSP48E1级联时延迟约7--8 ns。优化方案：将乘法结果寄存一拍（增加ST\_REQUANT\_MUL状态），与C1的ST\_SHIFT/ST\_CLAMP拆分思路一致。
\end{itemize}

综合来看，达到125 MHz需要在MAC累加与重量化乘法两处各增加一级流水寄存器，代价是每层推理增加约2个额外状态周期（对总延迟影响$<$0.1\%），但可将时钟频率从80 MHz提升至125 MHz（+56\%），在DMA路径下硬件compute从约4 ms降至约2.6 ms，端到端加速比可进一步提升。

\subsubsection{P0 — read\_output S2MM 优化}
\label{sec:p0_s2mm}

§\ref{sec:c3_bottleneck}的瓶颈分析指出，C3 DMA路径实测503.7 ms/img中，read\_output仍通过AXI4-Lite逐字MMIO读取（约257 ms，占61.7\%），这是仅次于weight load的第二大瓶颈。根本原因在于C3重构仅覆盖了MM2S方向（weight/input/bias写入），而结果回读路径仍沿用Legacy serial MMIO——每读一个INT8结果需一次\texttt{mmio.read()}调用（约50 $\mu$s），Conv1层576个输出像素$\times$6通道=3456字节，仅读回即需约173 ms。

为此，本工作实施了P0优化：新增AXI4-Stream master模块\texttt{cim\_axi\_stream\_source.sv}（约210行），将output\_buffer的读回路径从$N$次AXI4-Lite单字读替换为一次DMA S2MM批量传输。数据流为：

\begin{center}
\texttt{output\_buffer (BRAM)} $\rightarrow$ \texttt{cim\_axi\_stream\_source} $\rightarrow$ \texttt{M\_AXIS\_RESULT} $\rightarrow$ \texttt{axi\_dma/S\_AXIS\_S2MM} $\rightarrow$ \texttt{S\_AXI\_HP1} $\rightarrow$ \texttt{DDR}
\end{center}

模块采用5状态FSM（IDLE$\rightarrow$WAIT$\rightarrow$WARMUP$\rightarrow$READ$\rightarrow$SEND）处理BRAM的2-cycle读延迟与非4对齐末字打包。每4个INT8结果打包为一个32-bit AXI4-Stream beat，末拍置\texttt{tlast=1}告知DMA结束接收。Python驱动侧采用direct register mode（绕过PYNQ \texttt{\_SDMAChannel}），按PG021规范直接操作S2MM的DMACR/DA/LENGTH寄存器，并实现双缓冲交替机制避免缓冲区复用问题。

调试过程中解决了若干关键缺陷：（1）\texttt{is\_last\_word}粘滞位——FSM在传输完成后未清零，导致第二次及后续传输在首拍后立即终止；（2）\texttt{done}信号原为1-cycle pulse（16.67 ns），Python MMIO轮询无法捕获，改为粘滞信号（在下次\texttt{cfg\_start}时清零）；（3）非4字节对齐长度导致DMAIntErr——因source无\texttt{tkeep}信号，需将DMA LENGTH与source \texttt{cfg\_len}同步pad至4字节边界。

表\ref{tab:p0_benchmark}对比了P0前后的端到端性能，表\ref{tab:p0_latency}给出了P0后的LeNet-5逐项延迟分解。

\begin{table}[H]
    \centering
    \caption{P0 S2MM优化前后LeNet-5端到端性能对比（200张MNIST，PYNQ-Z2，60 MHz）}
    \label{tab:p0_benchmark}
    \begin{tabular}{lccc}
        \toprule
        指标 & C3 DMA（P0前） & C3 + P0 S2MM & 加速比 \\
        \midrule
        read\_out延迟 & 257 ms & 19.65 ms & \textbf{13.1×} \\
        单张平均延迟 & 503.7 ms & 128.4 ms & \textbf{3.9×} \\
        吞吐率 & 1.99 fps & 7.8 fps & \textbf{3.9×} \\
        分类准确率 & 99.5\%（199/200） & 99.5\%（199/200） & 一致 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    \centering
    \caption{P0 S2MM后LeNet-5单张推理延迟分解（128.4 ms/img, 60 MHz）}
    \label{tab:p0_latency}
    \small
    \begin{tabular}{lrr}
        \toprule
        阶段 & 延迟（ms） & 占比 \\
        \midrule
        setup（configure + load\_w + load\_b） & 41.05 & 32.0\% \\
        load\_x（per-MVM input） & 23.01 & 17.9\% \\
        im2col（Python侧） & 20.58 & 16.0\% \\
        read\_out（S2MM DMA） & 19.65 & 15.3\% \\
        dma\_x\_setup（stream sink arm） & 8.87 & 6.9\% \\
        compute（硬件MVM + 后量化） & 7.17 & 5.6\% \\
        dma\_x\_transfer & 5.55 & 4.3\% \\
        dma\_w\_transfer & 0.81 & 0.6\% \\
        dma\_w\_setup & 0.73 & 0.6\% \\
        dma\_b\_setup + transfer & 0.99 & 0.8\% \\
        \midrule
        \textbf{合计} & \textbf{128.4} & \textbf{100\%} \\
        \bottomrule
    \end{tabular}
\end{table}

P0优化将read\_out从257 ms降至19.65 ms（13.1×加速），端到端延迟从503.7 ms进一步降至128.4 ms（3.9×加速 vs pre-P0 DMA，13.2× vs MMIO baseline 1690.5 ms）。至此，数据通路的双向瓶颈（MM2S写入 + S2MM读回）均已通过AXI4-Stream + DMA方案解决。

当前的瓶颈已转移至：（1）\textbf{setup 41ms（32\%）}——每层重复的configure + load\_weights + load\_bias，可通过layer-wise预加载与CSR批量写入优化，目标降至约10 ms；（2）\textbf{load\_x 23ms（18\%）}——51次MVM调用中每次的stream sink arm + DMA transfer开销，可通过批量输入预打包减少DMA启动次数；（3）\textbf{im2col 21ms（16\%）}——纯Python循环构建col\_matrix，可通过NumPy stride\_tricks或Numba JIT加速至约5 ms。三者合计可从约85 ms优化至约25 ms，预计端到端可降至约60--80 ms/img（12--16 fps）。以下§\ref{sec:sw_optimization}--\ref{sec:phase_c}依次记录了在这些瓶颈方向上的实际优化过程与最终成果。

\subsection{软件侧全链路优化}
\label{sec:sw_optimization}

针对§\ref{sec:p0_s2mm}中识别的三大软件瓶颈（setup 41ms、load\_x 23ms、im2col 21ms），本工作实施了两轮软件侧优化。

\subsubsection{第一轮（SW v1）：im2col向量化 + Layer-wise Batching + MaxPool向量化}

\textbf{im2col向量化}是收益最大的单项优化。原实现在Python中逐输出像素构造im2col列（三重嵌套循环），每张LeNet-5图像需展开576（Conv1）+ 64（Conv2）= 640个滑窗位置，每次展开包含$C_{in}\times K_h\times K_w$个元素的列向量拼接。替换方案使用NumPy的\texttt{as\_strided}创建5维视图$(C_{in}, H_{out}, W_{out}, K_h, K_w)$，然后通过transpose + reshape一次性完成列展开：

\begin{equation}
    \text{col\_matrix} = \text{x\_padded.unfold}(2, K_h, 1).\text{unfold}(3, K_w, 1).\text{transpose}(...).\text{reshape}(C_{in}\times K_h\times K_w, -1)
\end{equation}

该变换将640次Python循环替换为单次NumPy C-level操作，实测将im2col从20.58 ms降至1.20 ms（\textbf{17.1×加速}），且输出与循环版本bit-exact一致。

\textbf{Layer-wise Batching}（setup优化）：原实现每张图像独立执行完整的"configure → load\_weights → load\_bias → load\_input → compute → read\_output"周期，对于200张图片，Conv1的权重（约170 KB）被重复加载200次。新增的\texttt{predict\_batch()}方法改为逐层批量处理：对每一层，先将全部200张图像的input打包为列表，一次性配置层参数并加载权重/偏置（仅1次），然后逐图像执行MVM推理。这使得setup从每张41.05 ms摊薄至0.60 ms（\textbf{68×加速}），关键前提是P0 S2MM的read\_output不再阻塞后续图像的compute启动。

\textbf{MaxPool向量化}：与im2col类似，将Python三层循环替换为\texttt{as\_strided} 5D视图 + \texttt{.max(axis=(3,4))}，从约53 ms降至1.27 ms。

SW v1将端到端延迟从128.4 ms降至73.5 ms（\textbf{1.75×加速}，13.6 fps），im2col + setup两项合计从61.6 ms降至1.8 ms。

\subsubsection{第二轮（SW v2）：ndarray输出 + 向量化解包}

P0 S2MM的\texttt{\_read\_output\_dma}原返回Python list of ints，Conv输出解包（将packed MVM结果按$C_{out}$拆分回各像素）使用逐元素Python for循环，导致"other"开销高达19.1 ms。SW v2改为返回numpy int8数组，解包操作替换为\texttt{reshape(batch\_size, C\_out).T}单次numpy赋值。实测"other"从19.1 ms降至4.9 ms（-69\%），总延迟从73.5 ms进一步降至56.2 ms（\textbf{1.3×加速}，17.8 fps）。

两轮软件优化累计将端到端延迟从128.4 ms降至56.2 ms（\textbf{2.3×加速}），且全程accuracy保持99.5\%不变。

\subsection{k\_pack容量扩展与MVM调用削减}
\label{sec:kpack_expansion}

§\ref{sec:sqmapping}的SQ-mapping优化中，打包因子$k_{\text{pack}}$受限于硬件参数$\text{MAX\_IN\_DIM}$和$\text{MAX\_OUT\_DIM}$。在初始配置（$\text{MAX\_IN\_DIM}=784$，$\text{MAX\_OUT\_DIM}=128$）下，LeNet-5的Conv1打包因子$k_{\text{pack}} = \min(\lfloor784/25\rfloor, \lfloor128/6\rfloor) = \min(31, 21) = 21$，Conv2为$k_{\text{pack}} = \min(\lfloor784/150\rfloor, \lfloor128/16\rfloor) = 5$，两卷积层合计需$28+13=41$次MVM调用。加上3个FC层，总计44次MVM/图。

扩大$\text{MAX\_IN\_DIM}$和$\text{MAX\_OUT\_DIM}$可以直接提升$k_{\text{pack}}$，减少MVM调用次数——每次MVM调用消除都节省了对应的DMA stream sink arm + transfer + wait开销（约0.5--1.0 ms/次）。本工作分两步扩展维度上限：

\begin{table}[H]
    \centering
    \caption{k\_pack容量扩展两阶段参数与实测结果（200张LeNet-5，PYNQ-Z2）}
    \label{tab:kpack_expansion}
    \small
    \begin{tabular}{lccccc}
        \toprule
        阶段 & MAX\_IN\_DIM & MAX\_OUT\_DIM & Conv1 $k_{\text{pack}}$ & Conv2 $k_{\text{pack}}$ & 总MVM次数 \\
        \midrule
        原始 & 784 & 128 & 21 & 5 & 44 \\
        v1 & 1024 & 256 & 40 & 6 & 29 \\
        v2 & 1536 & 256 & 42 & 10 & 24 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    \centering
    \caption{k\_pack扩展后端到端性能（200张LeNet-5，PYNQ-Z2，60 MHz）}
    \small
    \begin{tabular}{lcccc}
        \toprule
        阶段 & MVM次数 & 实测延迟（ms/img） & fps & 相对上一步加速 \\
        \midrule
        SW v2（784） & 44 & 56.2 & 17.8 & -- \\
        v1（1024） & 29 & 40.5 & 24.7 & 1.39× \\
        v2（1536） & 24 & 36.9 & 27.1 & 1.10× \\
        \bottomrule
    \end{tabular}
\end{table}

v2阶段BRAM占用率达到90\%（128/140 BRAM36），已接近PYNQ-Z2物理极限，无法继续增大参数。MVM总数从44降至24（\textbf{-45\%}），DMA启动开销同比例减少。分类准确率全程保持99.5\%（199/200）不变。

\subsection{C1 100MHz与TILE\_SPLIT\_FACTOR流水拆分}
\label{sec:c1_100mhz}

§\ref{sec:c1_pipeline}的80 MHz构建已时序收敛，但进一步提升至100 MHz时遇到瓶颈：即使C1后量化流水线拆分已将barrel shifter路径控制在约10 ns，16路MAC链（DSP48E1级联 + CARRY4进位链）的组合延迟仍约16 ns，在10 ns周期下无法闭合。

解决方案是将Tile内部16列MAC拆分为4组（每4列一组），分4个时钟周期顺序执行：

\begin{equation}
    \text{psum}[r] = \sum_{q=0}^{3} \sum_{c=4q}^{4q+3} \text{x\_eff}[c] \times w[r][c], \quad r = 0, \ldots, 15
\end{equation}

新增编译期参数\texttt{TILE\_SPLIT\_FACTOR=4}，FSM扩充为ST\_MAC\_Q0→Q1→Q2→Q3四拍状态（取代原ST\_MAC单状态）。每拍仅计算4个MAC + 单个CARRY4进位，组合路径从约16 ns降至约5 ns。额外改动包括：\texttt{input\_buffer}新增输出流水寄存器（断BRAM→x\_eff长路径），\texttt{psum\_accum}改为四quarter同拍累加，XDC约束对x\_eff\_reg设置MAX\_FANOUT=16。

100 MHz bitstream构建结果：WNS=-0.800 ns（801 failing endpoints），WHS=0.024 ns（MET），时序未完全收敛但上板功能正确。资源利用：LUT 30\%，Register 10\%，BRAM 90\%，DSP 100\%。相比60 MHz（WNS=-0.086 ns），100 MHz的负slack增大至-0.800 ns，关键路径位于x\_eff\_reg→DSP48E1→LUT/CARRY4→tile\_psum\_q0\_reg。室温下可稳定运行。

\begin{table}[H]
    \centering
    \caption{C1 100MHz vs 60MHz逐项延迟对比（200张LeNet-5，PYNQ-Z2）}
    \label{tab:c1_100mhz}
    \small
    \begin{tabular}{lccc}
        \toprule
        阶段 & 60 MHz（ms） & 100 MHz（ms） & 变化 \\
        \midrule
        load\_x & 12.97 & 13.33 & +2.8\% \\
        compute & 7.02 & 6.51 & \textbf{-7.3\%} \\
        read\_out & 10.41 & 10.61 & +1.9\% \\
        im2col & 1.20 & 1.20 & -- \\
        pool & 1.24 & 1.27 & +2.4\% \\
        setup & 0.60 & 0.60 & -- \\
        pack & 0.56 & 0.57 & +1.8\% \\
        other & 2.83 & 2.91 & +2.8\% \\
        \midrule
        \textbf{总计} & \textbf{36.90} & \textbf{37.06} & \textbf{+0.4\%} \\
        \bottomrule
    \end{tabular}
\end{table}

核心发现：\textbf{时钟频率提升被流水拆分抵消}。TILE\_SPLIT\_FACTOR=4使MAC运算从1周期扩展为4周期，时钟1.67×提升（60→100 MHz）被4×拆分抵消，MAC净吞吐率反而下降至$1.67/4 = 0.42\times$。非MAC流水级（bias/ReLU/requant/clamp）享受100\%频率收益，但占比太小（约15\%），使compute仅减少7.3\%。总延迟持平（+0.4\%，在测量噪声范围内）。

C1 100MHz的主要价值不在于延迟降低，而在于：（1）证明100 MHz时序在TILE\_SPLIT\_FACTOR=4下FPGA可运行；（2）为Phase B双缓冲（§\ref{sec:phase_b}）提供时钟裕量——DMA与compute重叠后，频率提升将直接转化为吞吐增益。

\subsection{Phase B: IBUF/OBUF双缓冲DMA↔Compute乒乓重叠}
\label{sec:phase_b}

\subsubsection{设计动机}

§\ref{sec:c1_100mhz}的逐项延迟分解揭示了最终瓶颈分布：load\_x（13.33 ms, 36.0\%）和read\_out（10.61 ms, 28.6\%）合计占端到端延迟的64.6\%，而compute仅占17.6\%。前续的C3/P0 DMA优化已经将单次传输延迟降至硬件下限（受限于DMA引擎带宽与AXI HP端口频率），进一步降低传输延迟的空间极为有限。

根本性的解决思路是\textbf{将DMA传输与硬件计算重叠执行}：当CIM核心正在使用当前输入/输出缓冲区进行计算时，DMA同时访问另一套缓冲区为下一次（或读取上一次）推理准备数据。这需要在input\_buffer和output\_buffer中各自实现双bank（bank0/bank1）乒乓结构。

\subsubsection{硬件实现}

设计原则为\textbf{最小化核心改动}：\texttt{cim\_accel\_core.sv}完全不修改（核心不知道bank的存在），bank选择逻辑完全封装在\texttt{cim\_axi\_lite\_slave.sv}中，通过单个CSR位控制。新增寄存器\texttt{reg\_ping\_ctrl}（地址0x06C），写1翻转（toggle），写0为no-op，读返回当前值。

Bank选择规则如表\ref{tab:phase_b_bank}所示：CIM compute始终使用\texttt{reg\_ping\_ctrl}选中的"活跃bank"（读IBUF活跃bank、写OBUF活跃bank），而DMA始终访问其反码对应的"非活跃bank"（写IBUF非活跃bank、读OBUF非活跃bank）。一次toggle操作即可原子性地交换DMA与compute的访问目标。

\begin{table}[H]
    \centering
    \caption{Phase B Bank选择逻辑}
    \label{tab:phase_b_bank}
    \begin{tabular}{lcl}
        \toprule
        信号 & 公式 & 用途 \\
        \midrule
...
RTL改动集中在4个文件：（1）\texttt{input\_buffer.sv}和\texttt{output\_buffer.sv}各自将单一\texttt{mem}数组替换为\texttt{bank0} + \texttt{bank1}双bank，新增\texttt{wr\_bank\_sel}和\texttt{rd\_bank\_sel}端口；（2）\texttt{cim\_axi\_lite\_slave.sv}新增\texttt{reg\_ping\_ctrl}寄存器和bank\_sel路由逻辑；（3）\texttt{cim\_pkg.sv}新增\texttt{CSR\_PING\_CTRL = 14'h06C}。BRAM从126增至128个BRAM36（+1.6\%），总占用率91.4\%，仍在PYNQ-Z2的140个上限之内。bitstream构建结果：WNS=-0.991 ns，WHS=0.010 ns，与100 MHz Phase A类似；0 DRC错误。
...
\begin{itemize}
    \item \texttt{toggle\_bank()}：写1到CSR\_PING\_CTRL，翻转bank选择；
    \item \texttt{start\_compute()}：非阻塞触发CIM计算（写CSR\_CTRL[0]=1），立即返回；
    \item \texttt{wait\_for\_done()}：阻塞轮询CSR\_STATUS[1]（done标志）。
\end{itemize}

原\texttt{start\_and\_wait()}内部改为：toggle\_bank → start\_compute → wait\_for\_done → toggle\_bank，确保DMA写入的数据进入活跃bank、compute结果落入DMA可读的非活跃bank。

核心编排逻辑\texttt{infer\_batch\_pingpong()}的乒乓序列如下：

\begin{enumerate}
    \item \textbf{Cold start}：load\_input(0) → toggle → start\_compute(0)。此时compute读取bank1的input[0]，DMA可访问bank0。
    \item \textbf{Overlap loop}（$i=1,\ldots,n-1$）：load\_input($i$)到bank0（DMA写入非活跃bank，与compute[$i-1$]重叠）→ wait\_for\_done($i-1$) → toggle（交换bank，compute[$i-1$]的结果现在在bank0，DMA可读取）→ start\_compute($i$)（立即启动，读取bank1的input[$i$]）→ read\_output($i-1$)从bank0（DMA读取非活跃bank，与compute[$i$]重叠）。
    \item \textbf{Tail}：wait\_for\_done($n-1$) → toggle → read\_output($n-1$)。
\end{enumerate}

关键设计决策：\texttt{start\_compute}在\texttt{read\_output}之前调用。这确保了read\_output的DMA S2MM传输与下一批compute完全重叠。对于Conv层，\texttt{predict\_batch()}将所有图像的所有$k_{\text{pack}}$批次收集为单一列表，一次\texttt{infer\_batch\_pingpong()}调用完成整个卷积层的全部MVM推理，最大化乒乓重叠的批次数量。

\subsubsection{实测结果}

\begin{table}[H]
    \centering
    \caption{Phase B前后LeNet-5端到端性能对比（200张MNIST，PYNQ-Z2，100 MHz）}
    \label{tab:phase_b_benchmark}
    \begin{tabular}{lccc}
        \toprule
        指标 & Phase A（100 MHz，无乒乓） & Phase B（100 MHz，乒乓） & 改善 \\
        \midrule
        单张平均延迟 & 37.06 ms & 29.18 ms & \textbf{-21.5\%} \\
        吞吐率 & 27.0 fps & 34.3 fps & \textbf{+27\%} \\
        load\_x & 13.33 ms & $\sim$0（隐藏） & -100\% \\
        read\_out & 10.61 ms & $\sim$0（隐藏） & -100\% \\
        im2col & 1.20 ms & 1.22 ms & -- \\
        pool & 1.27 ms & 1.24 ms & -- \\
        setup + pack & 1.17 ms & 1.25 ms & -- \\
        分类准确率 & 99.5\%（199/200） & 99.5\%（199/200） & 不变 \\
        \bottomrule
    \end{tabular}
\end{table}

Phase B将load\_x（13.33 ms, 36\%）和read\_out（10.61 ms, 28.6\%）完全隐藏在compute之后，端到端延迟从37.1 ms降至29.2 ms（-22\%）。剩余的29.2 ms中，$\sim$25.4 ms为所有MVM调用的乒乓wall time（max(DMA, compute)路径），$\sim$3.7 ms为im2col + pool + setup + pack等不可重叠的Python侧开销。

\subsubsection{优化历程总览}

表\ref{tab:optimization_journey}汇总了从MMIO基线到Phase B的完整优化历程，图\ref{fig:optimization_journey}以柱状图展示了各阶段的延迟演进。

\begin{table}[H]
    \centering
    \caption{LeNet-5端到端优化完整历程（200张MNIST，PYNQ-Z2）}
    \label{tab:optimization_journey}
    \small
    \begin{tabular}{lccccc}
        \toprule
        阶段 & 延迟（ms/img） & fps & 相对上一步 & 累计加速比 vs MMIO & 关键优化 \\
        \midrule
        MMIO baseline & 1690.5 & 0.59 & -- & 1.0× & -- \\
        C3 MM2S DMA & 503.7 & 1.99 & 3.4× & 3.4× & weight/input/bias DMA写入 \\
        P0 S2MM DMA & 128.4 & 7.8 & 3.9× & 13.2× & read\_output DMA读回 \\
        SW v1（向量化+batch） & 73.5 & 13.6 & 1.75× & 23.0× & im2col/maxpool向量化，layer-wise batch \\
        SW v2（ndarray） & 56.2 & 17.8 & 1.3× & 30.1× & ndarray输出，向量化解包 \\
        k\_pack v1（1024） & 40.5 & 24.7 & 1.39× & 41.7× & MAX\_IN\_DIM=1024 \\
        k\_pack v2（1536） & 36.9 & 27.1 & 1.10× & 45.8× & MAX\_IN\_DIM=1536 \\
        C1 100MHz & 37.1 & 27.0 & 0.99× & 45.6× & 100MHz + TILE\_SPLIT=4 \\
        Phase B 乒乓 & 29.2 & 34.3 & 1.27× & 57.9× & IBUF/OBUF双缓冲DMA重叠 \\
        \textbf{Phase C 融合} & \textbf{29.2} & \textbf{34.3} & \textbf{--} & \textbf{57.9×} & FC→FC OBUF→IBUF内部直拷（MLP +23\%） \\
        \bottomrule
    \end{tabular}
\end{table}

从MMIO到Phase C，累计端到端加速比达到\textbf{57.9倍}，吞吐率从0.59 fps提升至34.3 fps。全程分类准确率保持99.5\%不变，所有阶段输出与Python golden model bit-exact一致。

\begin{figure}[H]
    \centering
    % Placeholder for optimization journey bar chart
    \fbox{\parbox{0.9\textwidth}{\centering \textit{占位：优化历程柱状图，数据来自表\ref{tab:optimization_journey}。}}}
    \caption{LeNet-5端到端延迟优化历程（PYNQ-Z2，200张MNIST）}
    \label{fig:optimization_journey}
\end{figure}

\subsection{Phase C: Layer Fusion — OBUF→IBUF内部直拷消除FC→FC DMA往返}
\label{sec:phase_c}

\subsubsection{设计动机}

Phase B双缓冲将load\_x与read\_out隐藏在compute之后，消除了DMA传输的可见延迟。但对于连续全连接层（如MLP的FC1→FC2，LeNet-5的FC3→FC4→FC5），层间过渡仍然需要一次完整的S2MM（将FC1输出从OBUF读回DDR）加上一次MM2S（将数据从DDR写入IBUF作为FC2输入）的DMA往返。每次DMA往返包含Python侧的\texttt{s2mm.transfer()}调用、DMA引擎的S2MM+MM2S两段传输和对应的中断/轮询开销，合计约1--2 ms（受PYNQ DMA API开销主导）。

这一开销在LeNet-5中可忽略不计（3个FC层合计仅占端到端延迟的约1\%，~0.3 ms），但在纯FC网络（如MLP 784→128→10）中，FC→FC DMA往返直接占端到端延迟的约23\%（~0.55 ms out of 2.4 ms）。为此，本工作实施Phase C Layer Fusion优化：在硬件内部将OBUF内容直接拷贝至IBUF，绕过DDR往返。

\subsubsection{硬件实现}

Fusion FSM集成于\texttt{cim\_axi\_lite\_slave.sv}中，与CSR解码、DMA MUX、ping-pong bank控制共享同一个always\_ff块。其4状态设计如下：

\begin{enumerate}
    \item \textbf{F\_IDLE}：等待PS写\texttt{CSR\_FUSION\_CTRL[0]=1}启动融合。进入后立即置\texttt{fusion\_busy=1}，设OBUF读地址为0，转移到F\_WAIT\_MUX。
    \item \textbf{F\_WAIT\_MUX}：等待OBUF地址MUX（将\texttt{obuf\_rd\_addr}从compute路径切换到fusion路径）在一个NBA边沿后生效。同时将地址从0推进到1，避免下一拍重复读取bank[0]。转移到F\_PACK。
    \item \textbf{F\_PACK}：每个周期从OBUF读取1字节（\texttt{rd\_data = bank[byte\_idx]}），移位拼入128位tile word。每累积16字节（一个完整tile word），转移到F\_WRITE提交。
    \item \textbf{F\_WRITE}：将128位tile word写入IBUF对应地址（通过weight-SRAM风格的AXI写入通路），复位byte计数器。若未完成全部元素，地址加1后返回F\_PACK；否则回到F\_IDLE并置\texttt{fusion\_done=1}。
\end{enumerate}

\textbf{OBUF BRAM读流水线约束}：OBUF的BRAM采用registered read（1周期延迟），且地址选择在组合逻辑中完成（\texttt{always\_comb} MUX）。这意味着OBUF读需要一个额外的预热周期——F\_WAIT\_MUX在此周期内不仅要等待MUX传播，还要将地址从0推进到1。若地址保持为0不变，下一拍F\_PACK会因NBA语义（always\_comb在当前周期仍看到旧地址）而重复读取bank[0]，导致1字节移位错误。这一约束在v3--v9的7个迭代版本中经历了反复调试才最终解决（v10 PASS）。

\textbf{多FC层权重复用}：为支持连续FC层的权重在同一推理批次中共存，新增两个CSR基址寄存器：
\begin{itemize}
    \item \texttt{CSR\_WEIGHT\_BASE}（0x07C）：指定当前层权重在weight SRAM中的起始tile索引；
    \item \texttt{CSR\_BIAS\_BASE}（0x080）：指定当前层偏置在bias SRAM中的起始地址。
\end{itemize}

在FC链推理前，软件一次性将所有FC层的权重/偏置加载到互不重叠的SRAM地址区间（如FC1权重占tile 0--391，FC2占392--399），后续每层仅需写一次\texttt{CSR\_WEIGHT\_BASE/CSR\_BIAS\_BASE}即可切换，无需重新DMA搬运。

\subsubsection{软件编排}

\texttt{cim\_driver.py::CIMModel}新增两个方法：

\begin{itemize}
    \item \texttt{\_find\_fc\_groups()}：扫描\texttt{self.layers}列表，自动识别连续FC层链（2层及以上）。对于MLP（FC1→FC2）和LeNet-5（FC3→FC4→FC5），各返回一个fusion group。
    \item \texttt{\_preload\_fc\_chain(group\_layers)}：计算链中每层在weight/bias SRAM中的offset，一次性DMA加载所有权重/偏置。加载完成后，逐图像执行FC链推理（configure → load\_input → start\_and\_wait → fusion\_copy → configure\_next → start\_and\_wait → ... → read\_output\_after\_last）。
\end{itemize}

融合拷贝\texttt{copy\_output\_to\_input()}的协议如下：PS写\texttt{CSR\_FUSION\_LEN}（要拷贝的INT8元素数）和\texttt{CSR\_FUSION\_CTRL[0]=1}（启动），轮询\texttt{CSR\_FUSION\_STATUS[0]}等待完成。硬件以逐字节方式从OBUF读取数据（每周期1字节），在F\_PACK状态中移位拼装为128位tile word（16字节/tile），在F\_WRITE状态中提交写入IBUF。每个tile需17个周期（16拍F\_PACK读字节 + 1拍F\_WRITE提交），加上1拍F\_WAIT\_MUX预热。MLP FC1输出的128维INT8向量需$1 + 8\times17 = 137$个时钟周期（约2.3 $\mu$s @ 60 MHz），相比被替代的DMA往返（约1--2 ms）可忽略不计。

\subsubsection{实测结果}

\begin{table}[H]
    \centering
    \caption{Phase C Layer Fusion前后性能对比（200张图片，PYNQ-Z2，100 MHz）}
    \label{tab:phase_c_benchmark}
    \begin{tabular}{lccc}
        \toprule
        网络 & 无fusion（ms/img） & 有fusion（ms/img） & 改善 \\
        \midrule
        MLP（784→128→10） & 2.4 & 1.9 & \textbf{+23\%} \\
        LeNet-5（7层） & 29.2 & 29.2 & $\sim$0\% \\
        \bottomrule
    \end{tabular}
\end{table}

MLP网络因仅含两个FC层，FC1→FC2的DMA往返（S2MM读出 + MM2S写入）占端到端延迟的约23\%，fusion消除此开销后获得显著的加速。LeNet-5网络中FC3→FC4→FC5仅占总延迟的约1\%（~0.29 ms），且已在Phase B乒乓的max(DMA, compute)重叠中部分隐藏，因此端到端增益可忽略不计——但这验证了fusion在不影响整体性能的前提下功能正确。

Fusion bitstream（checkpoint5）构建结果：WNS=-0.911 ns，WHS=0.031 ns，LUT占用32.66\%，FF占用10.25\%，BRAM占用88.21\%（含Phase B双bank），DSP占用100\%。相比Phase B（WNS=-0.991 ns），fusion FSM的额外组合逻辑（OBUF地址MUX + 128位移位拼装）使WNS正向改善约0.08 ns——因为fusion逻辑与compute关键路径互斥（fusion仅在compute空闲时运行），综合器可以将二者映射到互不重叠的逻辑区域。
...
（6）\textbf{AXI4-Stream DMA数据通路重构与全链路优化}：通过十阶段递进优化将LeNet-5端到端延迟从1,690.5 ms/img降至29.2 ms/img（\textbf{57.9×端到端加速}，34.3 fps），累计加速比高于学术界同类FPGA CIM工作一到两个数量级：
\begin{itemize}
    \item \textbf{C3（MM2S DMA）}：将weight/input/bias的搬运路径从AXI4-Lite逐字MMIO替换为AXI4-Stream + axi\_dma批量传输，端到端从1,690.5 ms降至503.7 ms（3.4×加速）。
    \item \textbf{P0（S2MM DMA）}：新增\texttt{cim\_axi\_stream\_source}模块，将output\_buffer读回从$N$次串行MMIO读替换为单次DMA S2MM传输，read\_out从257 ms降至19.65 ms（13.1×加速），端到端降至128.4 ms（7.8 fps）。
    \item \textbf{SW v1/v2（软件侧全优化）}：im2col向量化（17.1×）、layer-wise batching（68×）、maxpool向量化、ndarray输出与向量化解包，端到端从128.4 ms降至56.2 ms（2.3×加速，17.8 fps）。
    \item \textbf{k\_pack容量扩展}：两次递增MAX\_IN\_DIM（784→1024→1536），MVM调用次数从44降至24（-45\%），端到端从56.2 ms降至36.9 ms（1.53×加速，27.1 fps），BRAM占用率达90\%（PYNQ-Z2物理极限）。
    \item \textbf{C1 100MHz + TILE\_SPLIT\_FACTOR=4}：将MAC 16路拆分为4+4+4+4四拍流水，100 MHz bitstream构建成功，虽因拆分抵消导致总延迟持平，但验证了高频时序可行性，为双缓冲提供了时钟裕量。
    \item \textbf{Phase B（IBUF/OBUF双缓冲乒乓）}：在input\_buffer和output\_buffer中实现双bank结构，通过单个CSR位（0x06C）控制bank切换，将load\_x（13.33 ms, 36\%）和read\_out（10.61 ms, 28.6\%）完全隐藏在compute之后。端到端从37.1 ms降至29.2 ms（-22\%，34.3 fps），累计vs MMIO加速比57.9×。全程accuracy 99.5\%不变。
    \item \textbf{Phase C（Layer Fusion — OBUF→IBUF内部直拷）}：在\texttt{cim\_axi\_lite\_slave.sv}中实现4状态融合FSM（F\_IDLE→F\_WAIT\_MUX→F\_PACK↔F\_WRITE），将FC→FC层间过渡的S2MM+MM2S DMA往返替换为硬件内部OBUF→IBUF直接拷贝（约137周期/层，2.3 $\mu$s @ 60 MHz）。新增\texttt{CSR\_WEIGHT\_BASE/CSR\_BIAS\_BASE}支持多FC层权重在SRAM中共存。Python驱动\texttt{CIMModel}自动检测连续FC层链并预加载权重。MLP（784→128→10）端到端从2.4 ms/img降至1.9 ms/img（+23\%）；LeNet-5因FC层仅占总延迟约1\%，增益可忽略不计，但验证了融合功能正确性。bitstream WNS=-0.911 ns，LUT 32.66\%，BRAM 88.21\%。
\end{itemize}
...
（2）\textbf{最大维度限制}：当前MAX\_IN\_DIM=1536、MAX\_OUT\_DIM=256的配置已使BRAM占用率达90\%（PYNQ-Z2物理极限），适用于MNIST尺度网络但无法支持CIFAR-10/ImageNet级别模型。后续需通过权重分片加载（超标层分多次加载，消除维度硬上限）或迁移至更大BRAM容量的平台来解决。

（3）\textbf{KV260移植}：Kria KV260搭载Zynq UltraScale+（xck26），相对PYNQ-Z2具有显著更多的LUT（234K vs 53K）、BRAM（144 vs 140个36Kb）和DSP（1248 vs 220个），理论上可支持\texttt{PAR\_OB=4}甚至\texttt{PAR\_OB=8}的更高并行度、更大的$k_{\text{pack}}$因子，以及更低TILE\_SPLIT\_FACTOR下的更高时钟频率。本课题的移植工作已覆盖KV260专用board file与时序约束文件的准备、Ubuntu 24.04镜像烧录与固件更新、Vivado项目TCL脚本的KV260变体（\texttt{kv260/hw/scripts/vivado\_build.sh}），详细迁移记录见\texttt{docs/kv260\_migration.md}。但由于AXI地址空间映射差异（PYNQ-Z2的\texttt{M\_AXI\_GP0}基址为\texttt{0x40000000}，而KV260的\texttt{M\_AXI\_HPM0\_FPD}默认映射到\texttt{0xA0000000}）、PMOD引脚布局不同以及PYNQ镜像的Python环境差异等问题，上板端到端推理的功能验证尚未完全闭环，这是本课题后续最直接的工作方向。一旦KV260路径完整打通，即可形成"PYNQ-Z2 (100 MHz, 34.3 fps) vs KV260 (目标100--200 MHz, PAR=4)"的跨平台性能对比，是完善论文性能评估章节的重要补充。
...
（6）\textbf{AXI4-Stream DMA数据通路优化}：本工作已完成从MMIO到Phase C融合的十阶段全链路优化（§\ref{sec:c3_dma}--\ref{sec:phase_c}），将LeNet-5端到端延迟从1,690.5 ms/img降至29.2 ms/img（57.9×加速）。当前剩余瓶颈为$\sim$25.4 ms的MVM乒乓wall time（max(DMA, compute)路径，受限于DMA带宽与TILE\_SPLIT\_FACTOR=4下的硬件compute时间）以及$\sim$3.7 ms的不可重叠Python开销（im2col 1.22ms + pool 1.24ms + pack 0.66ms + setup 0.59ms）。进一步优化方向包括：（a）将MaxPool硬件化，在CIM核心的ST\_STORE后增加pooling FSM，消除软件pool延迟并减少一次DMA round-trip；（b）Conv→Pool→Conv Layer Fusion——Phase C已完成FC→FC的OBUF→IBUF内部直拷（§\ref{sec:phase_c}），但Conv→Pool→Conv链路因im2col变换的介入（Pool输出需在PS侧做im2col展开后才能送入下一Conv），其层间数据搬运尚未消除。未来可将MaxPool硬件化后，进一步在PL内部实现Conv→Pool→Conv的直连，省去两次DMA round-trip；（c）将PYNQ-Z2上的成果移植到KV260平台，利用更大的BRAM（360+）和更快的fabric提升$k_{\text{pack}}$并降低TILE\_SPLIT\_FACTOR。当前29.2 ms/img（34.3 fps）已远超原预测的60--80 ms/img目标。
