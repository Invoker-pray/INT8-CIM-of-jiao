...
启用下一小节所述的SQ-mapping软件映射优化（§\ref{sec:sqmapping}）后，实际上板运行200张LeNet-5推理的总wall time为325.1秒（1.625秒/张），其中绝大部分时间消耗在Python驱动的MMIO数据搬运上，硬件计算本身仅占1.267 ms/张。
...
\begin{table}[H]
    ...
    \begin{tabular}{lccc}
        ...
        端到端wall time（200张）& 325.1 s & 339.3 s & $+4.4\%$ \\
        单张平均延迟 & 1,625.3 ms & 1,696.7 ms & $+4.4\%$ \\
        LeNet-5分类准确率 & 99.5\%（199/200）& 99.5\%（199/200）& 不变 \\
        \bottomrule
    \end{tabular}
\end{table}

端到端wall time仅增加4.4\%而非硬件延迟增加的9\%，与§\ref{sec:profiler}的profiler结论完全吻合：硬件计算仅占总延迟的0.4\%，主体是AXI4-Lite MMIO开销（与时钟频率无关）。分类准确率与60 MHz完全一致，进一步确认55 MHz版本在功能上与60 MHz版本bit-exact等价。
