...
当前的瓶颈已转移至：（1）\textbf{setup 41ms（32\%）}——每层重复的configure + load\_weights + load\_bias，可通过layer-wise预加载与CSR批量写入优化，目标降至约10 ms；（2）\textbf{load\_x 23ms（18\%）}——51次MVM调用中每次的stream sink arm + DMA transfer开销，可通过批量输入预打包减少DMA启动次数；（3）\textbf{im2col 21ms（16\%）}——纯Python循环构建col\_matrix，可通过NumPy stride\_tricks或Numba JIT加速至约5 ms。三者合计可从约85 ms优化至约25 ms，预计端到端可降至约60--80 ms/img（12--16 fps）。进一步配合§\ref{sec:c1_pipeline}的流水线拆分将频率提升至80--125 MHz，有望在PYNQ-Z2上实现$>$20 fps的实时推理。
...
（6）\textbf{AXI4-Stream DMA数据通路重构与优化}：通过三阶段优化将LeNet-5端到端延迟从1,690.5 ms/img降至128.4 ms/img（13.2×端到端加速）：
\begin{itemize}
    \item \textbf{C3（MM2S DMA）}：将weight/input/bias的搬运路径从AXI4-Lite逐字MMIO替换为AXI4-Stream + axi\_dma批量传输，端到端从1,690.5 ms降至503.7 ms（3.4×加速），瓶颈从MMIO搬运转移至read\_output（61.7\%）。
    \item \textbf{P0（S2MM DMA）}：新增\texttt{cim\_axi\_stream\_source}模块，将output\_buffer读回路径从$N$次串行MMIO读替换为单次DMA S2MM传输，read\_out从257 ms降至19.65 ms（13.1×加速），端到端进一步降至128.4 ms（3.9× vs pre-P0，7.8 fps）。关键技术点包括direct register mode绕过PYNQ \texttt{\_SDMAChannel}、BRAM 2-cycle延迟补偿、非4对齐末字打包、\texttt{is\_last\_word}粘滞位修复。
    \item \textbf{当前瓶颈}：setup（41ms, 32\%）、load\_x（23ms, 18\%）、im2col（21ms, 16\%）合计占66\%，可通过layer-wise预加载、批量输入预打包与NumPy向量化进一步优化，预计端到端可降至60--80 ms/img（12--16 fps）。
\end{itemize}
...
（6）\textbf{AXI4-Stream DMA数据通路优化}：本工作已完成了C3（MM2S DMA写入）与P0（S2MM DMA读回）两阶段AXI4-Stream DMA数据通路重构（§\ref{sec:c3_dma}--\ref{sec:p0_s2mm}），将LeNet-5端到端延迟从1,690.5 ms/img降至128.4 ms/img（13.2×加速）。当前瓶颈已转移至Python侧setup（41ms）与im2col（21ms）开销，进一步优化的方向包括：（a）将im2col用NumPy向量化或Numba JIT加速，预计可将setup + im2col从62 ms降至约15 ms；（b）实现layer-wise权重预加载与CSR批量写入，减少重复configure开销；（c）探索Ping-pong双缓冲实现DMA传输与硬件计算的重叠执行，目标端到端60--80 ms/img（12--16 fps）。此外，若将时钟频率从60 MHz提升至80--125 MHz（配合§\ref{sec:c1_pipeline}的流水线拆分），硬件compute时间可进一步从7.2 ms降至3--5 ms，结合上述软件优化，有望在PYNQ-Z2上实现$>$20 fps的实时推理。
