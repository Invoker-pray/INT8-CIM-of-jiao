...
需要澄清的是，TILE_SPLIT采用"乘法器全实例化 + 累加路径拆分"的机制：16×16=256个乘法器在所有配置下均组合实例化（SPLIT=4时仅把每行16列的累加链按4列一组拆为psum\_q0$\sim$q3、由FSM分4拍寄存累加），因此"吞吐率降为1/4"指的是单个tile产出一次完整256-MAC结果由1拍变为4拍，而非实例化乘法器减少。由于PYNQ-Z2仅有220块DSP48E1，综合器将其中220个乘法映射至DSP48（故表\ref{tab:util_final}中DSP占用达100\%），其余乘法（连同重量化级的32位乘法）溢出至LUT实现。
...
另外关于picorv32版本的计算阵列还有一点需要说明，32KB FW BRAM在扣除约4KB固件代码后，约余28KB用于权重、偏置与输入，至多可容纳约28K个INT8参数。例如784→32→10（约25K参数）尚可放入，而784→128→10（约102K参数）或LeNet-5则远超此容量——这正是PicoRV32模式仅验证784→16→10小型MLP、而把较大网络留给ARM模式的根本原因。若要在纯PL侧支持更大模型，需扩大FW BRAM容量，或改由片外存储分批载入权重（代价是引入额外搬运开销）。在当前项目中暂未涉及对这一部分的深入研究。
