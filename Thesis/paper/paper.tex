...
实验结果表明，本设计在100 MHz工作频率下，MLP网络（784→128→10）端到端延迟为1.97 ms/幅（507.6 fps，DMA+batch模式），LeNet-5 CNN（7层）端到端延迟为30.13 ms/幅（33.2 fps），相对AXI4-Lite逐字MMIO基线在相同100 MHz频率下实现约110倍累计加速，LeNet-5分类准确率99\%--100\%（MLP为99.5\%），全部硬件输出与自研的Python整数golden model逐元素bit-exact一致,验证了RTL实现与量化算法的数值等价性。在与ARM Cortex-A9纯软件（NumPy/BLAS）同口径的对比中，CIM硬件在计算核层面快约135倍（MLP）至三个数量级（small\_mlp），在计入全部DMA与调度开销的系统级端到端口径下仍快于ARM软件（MLP约2.25倍、small\_mlp约2.51倍、LeNet-5相对batched-BLAS约1.36倍）。PicoRV32控制模式在100 MHz下时序为marginal（WNS=$-$0.954 ns）但常温上板功能正确，且呈现出显著的轻量化优势：相比ARM模式反而减少322个LUT（17,042 vs 17,364）和2,228个FF（8,660 vs 10,888），仅增加6个BRAM用于固件/结果存储，以更少的逻辑资源实现了纯PL侧自主推理，200张图片验证准确率约96\%且全部logits与ARM路径bit-exact一致。系统占用LUT 32.64\%、BRAM 88.21\%、DSP 100\%，CIM IP动态功耗为0.546 W。
}
...
Experimental results show that at 100 MHz, the MLP network (784$\rightarrow$128$\rightarrow$10) achieves 1.97 ms/image end-to-end (507.6 fps, DMA+batch mode), while LeNet-5 CNN (7 layers) achieves 30.13 ms/image end-to-end (33.2 fps), representing an approximately 110$\times$ cumulative speedup over the AXI4-Lite per-word MMIO baseline at the same 100 MHz, with LeNet-5 classification accuracy of 99\%--100\% (MLP 99.5\%) and all hardware outputs are bit-exact (element-wise) with our in-house Python integer golden model, verifying numerical equivalence between the RTL implementation and the quantization algorithm. In an apples-to-apples comparison against pure ARM Cortex-A9 software (NumPy/BLAS), the CIM hardware is about 135$\times$ (MLP) to three orders of magnitude (small\_mlp) faster at the compute-core level, and remains faster than ARM software even at the system-level end-to-end caliber that includes all DMA and scheduling overhead (about 2.25$\times$ for MLP, 2.51$\times$ for small\_mlp, and 1.36$\times$ for LeNet-5 versus the batched-BLAS implementation). The PicoRV32 control mode runs functionally correctly at 100 MHz with marginal timing (WNS=$-$0.954 ns) and demonstrates a notable lightweight advantage: it uses 322 fewer LUTs (17,042 vs 17,364) and 2,228 fewer FFs (8,660 vs 10,888) than the ARM mode while adding only 6 BRAMs for firmware/result storage, achieving about 96\% accuracy on the 784$\rightarrow$16$\rightarrow$10 model with all logits bit-exact to the ARM path. The system utilizes 32.64\% LUTs, 88.21\% BRAM, and 100\% DSP, with the CIM IP consuming 0.546 W dynamic power.
}
...
值得注意的是，上述芯片级宏单元工作大多以ASIC流片或宏单元级别的能效、面积指标对比为主，而对"如何把一个CIM计算阵列接入完整的SoC数据通路、在真实硬件平台上端到端跑通一个神经网络、并保证逐元素数值正确"这一系统级问题着墨相对较少。在以FPGA为载体的神经网络推理加速方面，近年来也已有大量工作通过脉动阵列、INT8/低比特量化、数据复用与层间流水等手段提升端到端吞吐与能效\cite{r14}\cite{r15}，但其多以独立加速器为主，与CIM计算范式的结合仍有探索空间。对于以FPGA为载体的原型验证而言，除了计算核心本身的设计之外，权重/输入/输出的批量搬运效率、层与层之间的数据调度方式、以及软硬件数值一致性的可验证性，往往才是真正决定端到端吞吐率与结果可信度的关键环节——一个MVM算得再快，如果数据搬运成为瓶颈、或者无法证明硬件输出与软件参考逐位一致，其工程价值都会大打折扣。本课题正是面向这一相对薄弱的环节，强调在资源受限的小型FPGA上建立一套"算得对、搬得快、可复现"的完整工程闭环，而非单纯追求某一项孤立的峰值指标。
...
这里对零点的处理方式值得展开说明。原始像素是$[0,255]$的无符号UINT8，而权重是有符号INT8；若把UINT8直接按有符号解释再与权重相乘，会在像素值$\ge 128$时产生符号歧义。为此，硬件在从输入缓冲读出时先对$x\_tile$做零扩展（对应RTL中的\texttt{\{1'b0, x\_tile\}}），将其显式视为非负的9位值，再减去CSR配置的零点得到有效输入，即\,\texttt{x\_eff = \{1'b0, x\_tile\} - input\_zp}\,。对本文涉及的全部MLP与LeNet-5模型，输入侧均取$\text{input\_zp}=0$、权重采用对称量化（zp=0），因此$\text{x\_eff}$即像素值本身、范围$[0,255]$，不引入任何额外偏移项，乘法被简化为"非负9位$\times$有符号8位"，golden model严格复现同一无符号零点约定，从而与硬件输出逐位一致。需要补充的是，\texttt{input\_zp}被保留为按层可配置的CSR以兼容带非零输入零点的量化约定；在标准整数量化中，非零输入零点$z$所引入的交叉项$z\cdot\sum_{c} w[r][c]$会在离线标定时按输出通道折入INT32偏置，因此无论零点取何值，硬件都只需执行同一条"零点减法$\to$MVM$\to$加偏置"流水即可保持数值正确。在位宽方面，按照9位字段最坏的情形来说，单个$9\text{-bit}\times 8\text{-bit}$乘积最大不超过约$17\text{-bit}$，沿16列累加后理论上限约$21\text{-bit}$，因此用32位部分和寄存器存放绝不会溢出，留有充足裕量。之所以坚持把Tile写成纯组合阵列而不在内部插入寄存器，一方面是为了把全部时序边界集中到核心引擎的FSM中统一管理，使Tile成为一个"输入到部分和"的纯函数式抽象；另一方面是为了让综合器能够把这些规则的乘加结构识别并映射到FPGA内置的DSP48E1硬核上——每个DSP48E1可在一拍内完成一次有符号$18\times25$乘法与48位累加，对9位输入、8位权重的INT8 MAC而言绰绰有余，相比用查找表搭建乘法器，资源与时序都更优。
...
\begin{itemize}
	...
	\item 资源开销低且反直觉地轻量：PicoRV32核心约1,500个LUT，完整子系统（含bridge、UART TX、FW/Result BRAM接口）因去除了axi\_dma及关联AXI互连，总LUT反而比ARM模式净减少322个，以更少的逻辑资源实现更自主的控制；
...
\begin{table}[H]
	\centering
	\caption{ARM模式下MLP（784→128→10）200张端到端性能对比（两测试集均值）}
	\label{tab:mlp_arm_configs}
	\begin{tabular}{lcccc}
		\toprule
		配置 & 总时间（s） & ms/img & fps & 准确率 \\
		\midrule
		MMIO-only & 96.52 & 482.57 & 2.07 & 99.50\% \\
		DMA & 5.46 & 27.30 & 36.63 & 99.50\% \\
		DMA + batch & 0.39 & 1.97 & 507.6 & 99.50\% \\
		\bottomrule
	\end{tabular}
\end{table}

从MMIO-only到DMA+batch，端到端加速约245倍。在200张真实MNIST手写数字测试中，硬件输出与Python golden model逐元素完全一致（100\% bit-exact），分类准确率99.5\%（199/200）；1张错分与golden model预测一致，源于INT8量化精度损失。Layer Fusion启用后，FC1→FC2的层间延迟从DMA S2MM+MM2S往返的数十毫秒降至约1.5 $\mu$s。
...
\begin{table}[H]
	...
	\begin{tabular}{lrr}
		...
		阶段 & 延迟（ms/img） & 占比 \\
		\midrule
		MVM乒乓wall time（max(DMA, compute)） & 26.36 & 87.5\% \\
		im2col（向量化） & 1.23 & 4.1\% \\
		pool（向量化） & 1.31 & 4.4\% \\
		pack（预打包） & 0.62 & 2.1\% \\
		setup（摊薄） & 0.55 & 1.8\% \\
...
双缓冲乒乓机制已成功将DMA load\_x和read\_out隐藏在compute之后，当前的30.1 ms端到端延迟主要由所有MVM调用的乒乓wall time——即max(DMA传输时间, 硬件计算时间)的逐次累加——构成，达26.36 ms（占87.5\%）。在此之后，不可重叠的Python侧开销（im2col + pool + pack + setup）合计约3.8 ms，仅占总延迟的12.5\%。图\ref{fig:latency_pie}以饼图形式直观呈现了该延迟分解。需要说明的是，上述逐项分解为profiler对200张图片的逐图平均测量结果，各项合计约30.1 ms/幅，与表\ref{tab:arm_multi_model}中200张批量基准给出的30.13 ms/幅（33.2 fps）几乎完全一致。
...
\begin{table}[H]
	...
	\begin{tabular}{lccc}
		...
		\midrule
		AXI4-Lite逐字MMIO基线 & 3314.20 & 0.30 & 1.0× \\
		\quad + AXI4-Stream/DMA双向数据通路 & 167.05 & 6.0 & 19.8× \\
		\quad + 双缓冲乒乓 + Layer Fusion + 批量推理 & 30.13 & 33.2 & 110× \\
		\bottomrule
	\end{tabular}
\end{table}

从逐字MMIO基线到最终的DMA+批量配置，累计端到端加速比达到约110倍，吞吐率从0.30 fps提升至33.2 fps。最终配置分类准确率为99\%--100\%（两测试集分别为199/200与200/200），各配置输出均与Python golden model逐元素bit-exact一致（一致率100\%）。该优化历程的延迟递减与累计加速比演进如图\ref{fig:opt_waterfall}所示。

\begin{figure}[H]
	\centering
	\includegraphics[width=0.92\textwidth]{pics/4-7.png}
	\caption{LeNet-5端到端优化历程瀑布图。柱状（左轴）为三种数据通路配置下延迟逐步下降，折线（右轴）为相对AXI4-Lite逐字MMIO基线的累计加速比，最终达到约110$\times$并越过30~fps实时阈值；各配置输出均与Python golden model逐元素bit-exact一致（一致率100\%），最终配置分类准确率99\%--100\%。}
...
\begin{table}[H]
	\centering
	\caption{层融合消融对比（DMA+batch配置，100 MHz，200张MNIST，两测试集均值）}
	...
	\begin{tabular}{lccc}
		...
		\midrule
		MLP（784→128→10） & 2.24（446.3） & 1.97（507.6） & 1.14× \\
		LeNet-5（7层） & 31.00（32.3） & 30.13（33.2） & 1.03× \\
		\bottomrule
	\end{tabular}
\end{table}

层融合在两种模型上分别带来1.03×$\sim$1.14×的端到端加速，对应每幅约0.27$\sim$0.87 ms的延迟下降。以全连接层为主的MLP收益最大（1.14×，每幅省约0.27 ms），因其唯一的FC1→FC2层间边界在其较短的推理流水线中占比较高；LeNet-5的相对收益最小（1.03×，每幅省约0.87 ms），因其延迟由卷积与池化阶段主导，仅末端的FC层边界受益于融合。需要说明的是，此处测得的增量（毫秒量级）小于§2.8中单边界"数十毫秒$\to$约1.5\,$\mu$s"的对比：在已启用双缓冲乒乓的最终配置中，FC层间的DMA往返已被乒乓机制与计算重叠所隐藏，层融合在此之上进一步省去重叠后仍残留的往返开销与软件介入，因此其边际收益相对较小。两种模型在开/关层融合下分类准确率均保持不变（MLP 99.5\%、LeNet-5 99\%--100\%），印证层融合为无损的纯性能优化。
\section{加速比分析}
...
为定量评估CIM硬件相对纯软件实现的性能优势，本文在同一块PYNQ-Z2的ARM Cortex-A9处理器上，用NumPy/BLAS实现了与硬件\emph{逐元素bit-exact一致}的INT8整数推理，并对同一批200张MNIST测试图片逐图计时（每图预热后多次重复取均值）。软件基线脚本（\texttt{benchmark\_sw\_baseline.py}）直接读取硬件端到端实测CSV作为对照，并在两个独立测试集上各运行一次。为避免此前版本中口径混淆的问题，本节明确给出\emph{两种口径}的对比，二者结论一致——CIM硬件在两种口径下均显著快于ARM软件。

\textbf{口径一：纯计算对比（compute-only，对称口径）。} 硬件侧取性能计数器实测的纯MAC周期数除以100 MHz，软件侧取NumPy矩阵运算内核时间，两侧均不含数据搬运，是对等的"计算核"比较。结果如表\ref{tab:speedup_compute}所示。
...
\begin{table}[H]
	...
	\begin{tabular}{lccc}
		...
		\midrule
		MLP（784→128→10） & 4,430.8 & 32.8 & 135.0× \\
		small\_mlp（784→16→10） & 4,448.9 & 4.2 & 1{,}054.2× \\
		LeNet-5（7层，sequential） & 621{,}632.6 & 760.3 & 817.6× \\
		LeNet-5（7层，batched BLAS） & 40{,}985.4 & 760.3 & 53.9× \\
		\bottomrule
	\end{tabular}
	\label{tab:speedup_compute}
\end{table}

LeNet-5的卷积层比较分两种软件实现：sequential模式与硬件相同地按输出像素逐一调用矩阵乘法（与硬件执行模型等价，是更直接的对比），SW延迟约621.6 ms，计算核加速比约817.6倍；batched模式将所有像素合并为单次大矩阵乘法（充分利用ARM BLAS的最优软件实现上界），SW延迟约41.0 ms，计算核加速比约53.9倍。即便对比ARM上最优的batched-BLAS实现，CIM计算核仍快约54倍；对FC类网络（MLP/small\_mlp）则快两到三个数量级。

\textbf{口径二：系统级端到端对比（end-to-end，公平口径）。} 此前版本曾将"硬件计算核延迟"与"软件端到端延迟"混用，导致误判硬件未跑赢软件。本节予以纠正：硬件侧采用与软件\emph{完全同口径}的端到端逐图实测墙钟时间（表\ref{tab:arm_multi_model}中DMA+batch配置），软件侧为ARM上完整逐图推理时间，二者均为"从输入到输出"的系统级延迟。结果如表\ref{tab:speedup_e2e}所示。
...
\begin{table}[H]
	...
	\begin{tabular}{lccc}
		...
		\midrule
		MLP（784→128→10） & 4.43 & 1.97 & 2.25× \\
		small\_mlp（784→16→10） & 4.45 & 1.77 & 2.51× \\
		LeNet-5（vs batched-BLAS） & 40.99 & 30.13 & 1.36× \\
		LeNet-5（vs sequential） & 621.6 & 30.13 & 20.6× \\
		\bottomrule
	\end{tabular}
	\label{tab:speedup_e2e}
\end{table}

由表\ref{tab:speedup_e2e}可见，在与软件完全同口径的系统级端到端对比下，CIM硬件在三种网络上均快于ARM Cortex-A9软件：MLP约2.25倍、small\_mlp约2.51倍、LeNet-5相对ARM最优的batched-BLAS实现约1.36倍（相对与硬件同执行模型的sequential实现则达约20.6倍）。这一结果纠正了此前因口径混淆得出的"硬件未跑赢软件"的错误结论——实际上，即便计入DMA搬运与Python侧的im2col/打包/调度等全部系统开销，CIM硬件的端到端延迟仍低于ARM软件，且在计算核纯算力口径下领先两到三个数量级（表\ref{tab:speedup_compute}）。两种口径的对比如图\ref{fig:sw_vs_hw}所示。

\begin{figure}[H]
	\centering
	\includegraphics[width=0.85\textwidth]{pics/4-8.png}
	\caption{软件基线与硬件推理延迟对比（对数坐标，ARM Cortex-A9 vs CIM @ 100 MHz）。纯计算口径下MLP、small\_mlp、LeNet-5（sequential/batched）的计算核加速比分别为135$\times$、1{,}054$\times$、818$\times$/54$\times$；系统级端到端口径下（含全部DMA与调度开销）硬件相对ARM软件仍快约2.25$\times$（MLP）、2.51$\times$（small\_mlp）、1.36$\times$（LeNet vs batched-BLAS）。两种口径下硬件均快于软件。}
	\label{fig:sw_vs_hw}
\end{figure}

需要说明的是，软件基线与硬件运行的是\emph{完全相同的INT8整数算法}，二者逐图分类结果与全部中间张量bit-exact一致（脚本中已交叉校验准确率，两侧完全吻合），因此上述加速比是在数值结果完全等价前提下的纯性能对比。本设计的价值定位于\emph{计算核算力效率 + 系统级端到端速度优势 + 数值正确性与可复现的工程闭环}：在资源受限的PYNQ-Z2小型SoC上，CIM硬件不仅在计算核层面相对CPU快两到三个数量级，在计入全部数据搬运开销的系统级端到端口径下仍保持对ARM软件的速度优势。进一步的性能提升空间（如§\ref{sec:future_work}所述DSP48 INT8 SIMD打包带来的compute吞吐翻倍、更大批量与片上权重驻留以进一步摊薄DMA）将使这一优势继续扩大。
...
\begin{table}[H]
	\centering
	\caption{ARM模式多模型多配置端到端性能汇总（100 MHz，200张MNIST，两测试集均值）}
	...
	\begin{tabular}{llcccc}
		...
		\midrule
		\multirow{2}{*}{small\_mlp（784→16→10）} & MMIO & 16.11 & 80.56 & 12.4 & 95.5--96.5\% \\
		& DMA+batch & 0.35 & 1.77 & 566.4 & 95.5--96.5\% \\
		\midrule
		\multirow{3}{*}{MLP（784→128→10）} & MMIO & 96.52 & 482.57 & 2.1 & 99.50\% \\
		& DMA & 5.46 & 27.30 & 36.6 & 99.50\% \\
		& DMA+batch & 0.39 & 1.97 & 507.6 & 99.50\% \\
		\midrule
		\multirow{3}{*}{LeNet-5（7层）} & MMIO & 662.84 & 3,314.20 & 0.30 & 99.50\% \\
		& DMA & 33.41 & 167.05 & 6.0 & 99.50\% \\
		& DMA+batch & 6.03 & 30.13 & 33.2 & 99.50\% \\
		\bottomrule
	\end{tabular}
\end{table}

从表可见：（1）DMA相比MMIO的加速比随模型规模增大而增大——MLP约17.7倍、LeNet-5约19.8倍，因数据搬运量越大DMA的批量优势越明显；（2）DMA+batch（双缓冲乒乓+层融合）在DMA基础上进一步加速——MLP约13.9倍、LeNet-5约5.5倍，将数据搬运延迟隐藏在计算之后，small\_mlp由MMIO到DMA+batch的累计加速约45.5倍；（3）各配置的硬件输出与Python golden model始终保持逐元素bit-exact一致（一致率100\%），即数据通路重构不引入任何计算误差；表中分类准确率为INT8量化模型相对标签的分类精度，由模型量化本身决定，与硬件数据通路无关（两测试集上MLP均为99.5\%，LeNet-5为99\%--100\%，small\_mlp约96\%）。
...
\begin{table}[H]
	...
	\begin{tabular}{lc}
		...
		\midrule
		测试图片数 & 200（×2测试集） \\
		分类准确率（vs 标签） & 95.50\%--96.50\%（191--193/200） \\
...
200张图片的全部10维logits输出均与ARM控制模式下相同模型的预测结果完全一致（bit-exact），计算错误为零。分类错误均源于784→16→10模型自身的INT8量化精度损失（两测试集分别为191/200与193/200，约95.5\%--96.5\%），验证了RISC-V固件控制逻辑的正确性。

PicoRV32模式在small\_mlp（784→16→10）上的端到端实测性能为：总时间约2.55 s（200张，两测试集均值），平均12.75 ms/img（约79.1 fps），准确率约96\%，全部logits与ARM路径bit-exact一致。相比ARM模式下相同模型的DMA+batch配置（1.77 ms/img），PicoRV32的延迟约为7.2倍，主要差距来自PicoRV32缺少DMA批量传输能力——权重与输入数据需由CPU逐字写入CIM CSR（等效于ARM MMIO路径），而非通过AXI-Stream DMA批量搬运。该差距是控制路径（轻量RISC-V软核 vs ARM Cortex-A9 + DMA引擎）的固有差异，而非CIM计算核心的性能差异。事实上，两种模式运行的是同一份CIM IP RTL，对同一small\_mlp模型的硬件MVM计算周期数完全相同，端到端差异完全来自数据搬运方式。逐图延迟统计上，PicoRV32的p50约5.9--6.0 ms、均值约6.0--6.1 ms、最小约5.8 ms、最大约15.6--15.7 ms（首图含权重加载开销）。
...
\begin{table}[H]
	...
	\begin{tabular}{lcc}
		...
		LUT占用 & 17,364 (32.64\%) & 17,042 (32.03\%) \\
		BRAM占用 & 123.5 (88.21\%) & 129.5 (92.50\%) \\
		small\_mlp延迟 & 1.77 ms/img（DMA+batch） & 12.75 ms/img \\
		small\_mlp吞吐 & 566.4 fps & 79.1 fps \\
...
如§\ref{sec:two_extremes}所述，本设计的输出并行度由编译期参数PAR\_OB控制：核心通过\texttt{generate}对PAR\_OB份16$\times$16 Tile完全参数化例化。需要特别澄清的是，板级实现固定PAR\_OB=1\emph{并非架构缺陷，而纯粹是PYNQ-Z2资源约束所致}——单个Tile的256个INT8乘加已使DSP48E1占用率达到100\%（见表\ref{tab:util_final}），物理上无法再容纳第二份Tile。换言之，PAR\_OB=1是该器件的资源天花板，而非设计能力上限。
...
扫描得到两个明确结论：其一，\emph{五种并行度配置全部成功elaborate且仿真输出与golden model 100\% bit-exact一致}（errors=0/256），证明阵列架构在PAR\_OB提升至16时仍完全正确、可综合——板级只跑PAR\_OB=1确系DSP资源所限，而非架构无法支持更高并行度。其二，实测加速比\emph{次线性}：并行度提升16倍仅换来约1.80倍的周期下降。

\subsection{加速比次线性的成因分析}

次线性的根因可由FSM的周期构成精确解释。单个输出分组（ob\_group）的总周期由两部分构成：

\textbf{（A）乘加阶段（可并行）。} 权重fetch、x\_eff锁存、4个quarter MAC子状态（TILE\_SPLIT=4）与psum累加，对N\_IB个输入块循环执行。该阶段在所有PAR\_OB份Tile上\emph{并行}完成，因此其总周期随PAR\_OB增大而减小：分组数为N\_OB/PAR\_OB，故乘加总周期从PAR\_OB=1的约2592周期降至PAR\_OB=16的约642周期。

\textbf{（B）输出/requant阶段（串行）。} 偏置加法、激活、requant、移位、钳位、写回OBUF构成逐神经元的7级流水（ST\_BIAS\_ADD$\to\cdots\to$ST\_WRITE\_OBUF），对每个输出神经元\emph{逐一串行}处理。该阶段每个分组处理PAR\_OB$\times$16个神经元，而分组数为N\_OB/PAR\_OB，故输出阶段总周期为$\frac{N\_OB}{PAR\_OB}\times PAR\_OB\times 16\times 7 = N\_OB\times 16\times 7$，约1792周期——\emph{与PAR\_OB完全无关，是不可被并行度压缩的固定开销}。
...
\textbf{（1）输出流水线化（最关键）。} 当前输出阶段每个神经元独占7个时钟周期（逐级走完流水），若将其改造为真正的吞吐流水线——每周期接纳一个神经元、7级填充后每周期输出一个结果——则输出阶段从$N\_OB\times16\times7$降至约$N\_OB\times16\times1$。据周期模型估算，仅此一项即可将PAR\_OB=16的加速比从1.80倍提升至约3.25倍。

\textbf{（2）权重fetch与计算重叠。} 当前每个输入块需经ST\_FETCH/ST\_WAIT\_SRAM串行取权重（约$2\times$PAR\_OB周期），可通过预取（在当前块计算时fetch下一块权重）将其隐藏。结合输出流水线化，PAR\_OB=16的加速比可进一步提升至约5.3倍。

\textbf{（3）输出阶段的并行化。} 更彻底的方案是让多个Tile的输出共享多套并行的requant单元，使输出阶段也随PAR\_OB缩放；代价是额外的requant逻辑面积，需与DSP/LUT预算权衡。

\textbf{（4）配合DSP48 INT8 SIMD打包释放并行资源。} 如§\ref{sec:future_work}所述，利用单DSP48E1的25$\times$18乘法器同时计算两路INT8乘法可将DSP用量减半，从而在同一PYNQ-Z2上真正例化PAR\_OB=2。需注意：唯有在上述输出流水线化改造之后，提升的并行度才能转化为可观的端到端加速，否则收益将被固定的输出开销吞没。

综上，本节通过周期级仿真确证了阵列架构在PAR\_OB=1$\sim$16下均\emph{可综合且功能完全正确}，板级PAR\_OB=1仅受DSP资源约束；同时定量揭示了当前加速比次线性的根因（串行输出流水构成约占半数的固定开销），并给出了从架构层面提高并行度收益的明确路径。
% ====================================================================
\chapter{总结与展望}
% ====================================================================
...
（2）数据通路优化：设计了AXI4-Stream + DMA双向批量传输架构，将数据搬运从逐字MMIO替换为DMA批量传输；实现了输入/输出缓冲双bank乒乓结构，将DMA传输与硬件计算重叠执行；针对连续FC层设计了OBUF→IBUF内部硬件直拷的层融合机制，消除层间DMA往返。累计将LeNet-5端到端延迟从3,314.20 ms/img降至30.13 ms/img（约110×加速，33.2 fps）。在与ARM Cortex-A9纯软件同口径的系统级端到端对比中，CIM硬件在三种网络上均快于软件（MLP约2.25×、small\_mlp约2.51×、LeNet-5相对batched-BLAS约1.36×），在计算核纯算力口径下更领先两到三个数量级。

（3）软件映射优化：引入并适配了块对角权重打包（SQ-mapping）这一已有的卷积映射方法，通过将多个输出像素的im2col列合并到同一次MVM调用，将小卷积核场景下的MVM调用次数削减约90\%（Conv1 97.6\%、Conv2 89.1\%），单次MVM维度占用率从不足2\%提升至约60\%--98\%。同时通过im2col/MaxPool向量化、layer-wise批量化等手段将Python侧开销压缩至总延迟的约12.5\%。
...
（5）软核集成与轻量化：成功集成PicoRV32 RISC-V软核替代ARM PS进行推理控制，CIM IP完全不修改。相比ARM模式，PicoRV32模式展现出反直觉的轻量化优势：总LUT减少322个（17,042 vs 17,364）、FF减少2,228个（8,660 vs 10,888），仅BRAM增加6个，在100 MHz下时序为marginal（WNS=$-$0.954 ns）但常温上板功能正确。这一结果说明用轻量RISC-V软核（约1,500 LUT）替代复杂的DMA IP及关联Stream/Interconnect（合计超5,000 LUT/FF）是资源效率更优的纯PL推理方案。200张MNIST图片验证准确率约96\%（191--193/200），全部logits与ARM路径bit-exact一致。
...
（1）DSP占用率与并行度：当前设计DSP48E1占用率达到100\%，使板级实现只能例化PAR\_OB=1。需强调这是器件资源约束而非架构限制——§\ref{sec:par_ob_sweep}的VCS周期级仿真已证明阵列在PAR\_OB=1$\sim$16下均可综合且功能100\% bit-exact正确。后续可通过DSP48 INT8×2 SIMD打包技术（利用单DSP的25×18乘法器同时计算两个独立INT8乘法，将DSP用量减半）释放资源用于PAR\_OB=2扩展。但如§\ref{sec:par_ob_sweep}的成因分析所示，单纯提升并行度的收益受串行输出/requant流水（约占半数的固定开销）所限而呈次线性；唯有先将输出阶段流水线化（预期可使PAR\_OB=16的加速比从1.80倍提升至约3.25倍，叠加权重预取后约5.3倍），提升的并行度才能充分转化为端到端加速。二者结合是后续提升硬件compute吞吐的关键路径。
...
\includepdf[pages=-,scale=0.95,offset=0 -10,pagecommand={\thispagestyle{plain}}]{appendix/appendix_a_original.pdf}

\xjtuappendixchapter{外文翻译（译文）}
\chaptermark{附\quad 录\ B}
\includepdf[pages=-,scale=0.95,offset=0 -10,pagecommand={\thispagestyle{plain}}]{appendix/appendix_b_translation.pdf}
