...
\begin{figure}[H]
	...
	\caption{软件基线与硬件推理延迟对比（对数坐标，ARM Cortex-A9 @ 650 MHz vs CIM @ 100 MHz）。MLP、LeNet-5（sequential）、LeNet-5（batched）三组工作负载的\emph{计算核}加速比分别为43.0$\times$、34.8$\times$与6.0$\times$（HW为性能计数器实测的纯MAC计算核延迟,SW为ARM上NumPy/BLAS矩阵运算内核时间;两者均为compute-only口径,不含数据搬运）。}
	\label{fig:sw_vs_hw}
\end{figure}

需要强调的是，表\ref{tab:speedup}为"硬件计算核延迟 vs 软件端到端延迟"的口径对比：HW列仅统计性能计数器读到的纯MAC计算周期，不含DMA搬运与Python侧的im2col/打包/调度开销，SW列为 ARM Cortex-A9 上 NumPy/BLAS 矩阵运算内核的逐图计时(预热后取多次中位数),与硬件计算核的纯 MAC 周期同为 compute-only 口径。因此43.0×/34.8×/6.0×反映的是CIM\emph{计算核}相对CPU软件实现的算力优势，而非系统级端到端加速。若改用同口径的端到端对比（表\ref{tab:arm_multi_model}中DMA+batch实测）：MLP为1.89 ms（vs 软件1.41 ms，约0.75×）、LeNet-5为28.48 ms（vs sequential 26.47 ms约0.93×、vs batched-BLAS 4.53 ms约0.16×）。可见在本原型的系统级延迟上，受逐次MVM调用的DMA批量搬运主导（见表\ref{tab:latency_breakdown}，MVM乒乓wall time占87.5\%且本质为DMA传输时间），当前实现尚未跑赢ARM Cortex-A9软件基线。这一结果符合预期：在PYNQ-Z2这类小型SoC上，单次MVM调用的DMA与软件调度固定开销与CPU逐调用开销量级相当，计算核虽快约43倍，但小模型下其收益被数据通路开销抵消。要在系统级超越CPU，需进一步压缩单次调用的搬运/调度开销——例如更大批量与片上权重驻留以摊薄DMA、以及§\ref{sec:future_work}所述DSP48 INT8 SIMD打包带来的compute吞吐翻倍。因此本设计的价值定位于\emph{计算核算力效率 + 端到端数值正确性与可复现的工程闭环}，而非在该小型SoC上对CPU的系统级延迟超越。
...
在PYNQ-Z2上针对784→16→10小型MLP模型（INT8量化，模型数据约13.3KB）进行了200张MNIST手写数字图片的端到端推理验证，结果如表\ref{tab:rv32_verify}所示。
