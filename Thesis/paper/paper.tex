...
实验结果表明，本设计在100 MHz工作频率下，MLP网络（784→128→10）端到端延迟为2.00 ms/幅（499.6 fps，DMA+batch模式），LeNet-5 CNN（7层）端到端延迟为29.91 ms/幅（33.4 fps），相对AXI4-Lite逐字MMIO基线在相同100 MHz频率下实现约109倍累计加速，200张MNIST测试图片分类准确率99.5\%，全部输出与Python bit-accurate参考模型逐元素一致。PicoRV32控制模式在100 MHz下时序为marginal（WNS=$-$0.694 ns）但常温上板功能正确，且呈现出显著的轻量化优势：相比ARM模式反而减少351个LUT（17,023 vs 17,374）和2,317个FF（8,594 vs 10,911），仅增加6个BRAM用于固件/结果存储，以更少的逻辑资源实现了纯PL侧自主推理，200张图片验证准确率96.50\%且全部logits与ARM路径bit-exact一致。系统占用LUT 32.66\%、BRAM 88.21\%、DSP 100\%，CIM IP动态功耗为0.559 W。
...
这里对零点的处理方式值得展开说明。原始像素是$[0,255]$的无符号UINT8，而权重是有符号INT8；若直接把两者相乘再累加，符号处理较为繁琐。本设计选择令$\text{input\_zp}=-128$，即把输入统一"加上128"映射到$[128,383]$这一全为非负的9位区间，硬件在做符号扩展时只需在高位补零（对应RTL中的\texttt{\{1'b0, x\_tile\}}），从而把"有符号$\times$有符号"简化为"非负9位$\times$有符号8位"，既保证了与软件参考模型逐位一致，又简化了乘法器前端的符号逻辑——这一无符号约定也是后文golden model必须严格复现的细节之一。在位宽方面，单个$9\text{-bit}\times 8\text{-bit}$乘积最大不超过约$17\text{-bit}$，沿16列累加后理论上限约$21\text{-bit}$，因此用32位部分和寄存器存放绝不会溢出，留有充足裕量。之所以坚持把Tile写成纯组合阵列而不在内部插入寄存器，一方面是为了把全部时序边界集中到核心引擎的FSM中统一管理，使Tile成为一个"输入到部分和"的纯函数式抽象；另一方面是为了让综合器能够把这些规则的乘加结构识别并映射到FPGA内置的DSP48E1硬核上——每个DSP48E1可在一拍内完成一次有符号$18\times25$乘法与48位累加，对9位输入、8位权重的INT8 MAC而言绰绰有余，相比用查找表搭建乘法器，资源与时序都更优。
...
这一"先攒齐128位、再整行写入"的read-modify-write式通路，本质上是在32位的AXI接口与128位的BRAM整行写入之间架设了一个宽度转换桥：上层软件仍按32位字（chunk）逐个送权重，硬件在staging寄存器里把连续4个chunk拼成一整行，只在第4个chunk到达时拉高一拍写使能、把整行一次性写进BRAM。这样既保持了对软件友好的32位写接口（与既有的\texttt{weight\_to\_chunks}打包格式兼容），又满足了综合器对BRAM"整字写、不做位选择部分写"的苛刻要求。是否成功推断BRAM对本设计而言并非锦上添花而是生死攸关：权重存储的最坏容量约为$16\times128\times1536/8\approx 384\text{ KB}$，若退化为寄存器实现，所需的触发器数量将远超xc7z020全片10万余个FF的规模，整个工程根本无法布通；而一旦正确推断为BRAM，这部分存储几乎不占用LUT/FF，仅消耗Block RAM资源。因此这一拆分既是功能正确性的保证，也是资源可行性的前提。
...
第一层：Python bit-accurate参考模型。\texttt{golden\_model.py}以纯整数算术实现与硬件完全相同的计算流程（零点减法、MVM、偏置累加、ReLU、重量化），输出hex格式测试向量供RTL仿真和板级验证使用。
...
\begin{table}[H]
	...
	\begin{tabular}{lcccc}
		...
		DMA + batch & 0.40 & 2.00 & 499.6 & 99.00\% \\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{lccc}
		...
		\quad + 双缓冲乒乓 + Layer Fusion + 批量推理 & 29.91 & 33.4 & 109× \\
		\bottomrule
	\end{tabular}
\end{table}

从逐字MMIO基线到最终的DMA+批量配置，累计端到端加速比达到约109倍，吞吐率从0.31 fps提升至33.4 fps。最终配置分类准确率为99.5\%（199/200），各配置输出均与Python golden model逐元素bit-exact一致（一致率100\%）。该优化历程的延迟递减与累计加速比演进如图\ref{fig:opt_waterfall}所示。
...
MLP的SW延迟约1411 $\mu$s主要由FC1层（784×128，1368 $\mu$s）主导；HW延迟32.8 $\mu$s来自性能计数器实测3282周期，加速比约43.0倍。

LeNet-5的比较分两种模式：sequential模式与硬件相同地按输出像素逐一调用矩阵乘法（conv1 576次、conv2 64次），SW延迟约26.5 ms，加速比约34.8倍；batched模式将所有像素合并为单次矩阵乘法调用（BLAS路径），SW延迟约4.5 ms，加速比约6.0倍。Sequential模式是更公平的对比（与硬件计算模式等价），batched模式代表充分利用ARM CPU的最优SW实现上界。两种模式下CIM硬件均具有明显加速优势，对比结果如图\ref{fig:sw_vs_hw}所示。
...
\begin{table}[H]
	...
	\begin{tabular}{llcccc}
		...
		& DMA+batch & 0.39 & 1.93 & 519.3 & 96.50\% \\
		\midrule
		\multirow{3}{*}{MLP（784→128→10）} & MMIO & 99.68 & 498.41 & 2.0 & 99.00\% \\
		& DMA & 5.59 & 27.93 & 35.8 & 99.00\% \\
		& DMA+batch & 0.40 & 2.00 & 499.6 & 99.00\% \\
		\midrule
		\multirow{3}{*}{LeNet-5（7层）} & MMIO & 652.21 & 3,261.06 & 0.31 & 99.50\% \\
		& DMA & 34.14 & 170.68 & 5.86 & 99.50\% \\
		& DMA+batch & 5.98 & 29.91 & 33.4 & 99.50\% \\
		\bottomrule
	\end{tabular}
\end{table}
...
（2）数据通路优化：设计了AXI4-Stream + DMA双向批量传输架构，将数据搬运从逐字MMIO替换为DMA批量传输；实现了输入/输出缓冲双bank乒乓结构，将DMA传输与硬件计算重叠执行；针对连续FC层设计了OBUF→IBUF内部硬件直拷的层融合机制，消除层间DMA往返。累计将LeNet-5端到端延迟从3,261.06 ms/img降至29.91 ms/img（约109×加速，33.4 fps）。

（3）软件映射优化：引入并适配了块对角权重打包（SQ-mapping）这一已有的卷积映射方法，通过将多个输出像素的im2col列合并到同一次MVM调用，将小卷积核场景下的阵列利用率提升至接近100\%，MVM调用次数削减89\%以上。同时通过im2col/MaxPool向量化、layer-wise批量化等手段将Python侧开销压缩至总延迟的12.7\%。
...
（5）软核集成与轻量化：成功集成PicoRV32 RISC-V软核替代ARM PS进行推理控制，CIM IP完全不修改。相比ARM模式，PicoRV32模式展现出反直觉的轻量化优势：总LUT减少351个（17,023 vs 17,374）、FF减少2,317个（8,594 vs 10,911），仅BRAM增加6个（FW/Result双端口BRAM），在100 MHz下时序为marginal（WNS=$-$0.694 ns）但常温上板功能正确。这一结果说明用轻量RISC-V软核（约1,500 LUT）替代复杂的DMA IP及关联Stream/Interconnect（合计超5,000 LUT/FF）是资源效率更优的纯PL推理方案。200张MNIST图片验证准确率96.50\%（193/200），全部logits与ARM路径bit-exact一致。
...
（2）最大维度限制：当前MAX\_IN\_DIM=1536、MAX\_OUT\_DIM=256的配置已使BRAM占用率达88.2\%（PYNQ-Z2物理极限），适用于MNIST尺度网络但无法支持CIFAR-10/ImageNet级别模型。后续需通过权重分片加载（超标层分多次加载，消除维度硬上限）或迁移至更大BRAM容量的平台来解决。
