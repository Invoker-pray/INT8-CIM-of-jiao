...
\begin{figure}[H]
	...
	\caption{数字SRAM-CIM两种实现的结构对比。左：比特级bit-serial popcount架构，逼近"存算一体"本义但单MAC需8拍且FPGA不友好；右：字级行为建模MAC架构（本课题），单拍完成MAC、易于映射到DSP48并在FPGA上闭合时序。另外以上图片用较小的尺寸仅作为示意，实际设计为16x16。}
