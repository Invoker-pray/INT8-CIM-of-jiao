...
LeNet-5的比较分两种模式：sequential模式与硬件相同地按输出像素逐一调用矩阵乘法（conv1 576次、conv2 64次），SW延迟约26.5 ms，加速比约34.8倍；batched模式将所有像素合并为单次矩阵乘法调用（BLAS路径），SW延迟约4.5 ms，加速比约6.0倍。Sequential模式是更公平的对比（与硬件计算模式等价），batched模式代表充分利用ARM CPU的最优SW实现上界。两种模式下CIM计算核相对CPU软件实现均具有明显算力优势（系统级端到端口径见下），对比结果如图\ref{fig:sw_vs_hw}所示。
