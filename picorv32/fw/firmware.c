...
static void cim_set_weight_base(uint32_t tile_offset) { CIM_WEIGHT_BASE = tile_offset; }
static void cim_set_bias_base(uint32_t word_offset)   { CIM_BIAS_BASE = word_offset; }
...
static void cim_load_weights_at(const uint32_t *chunks, uint32_t n, uint32_t start_tile) {
    CIM_WDMA_ADDR = start_tile;  CIM_WDMA_CTRL = 0x02;
    for (uint32_t i = 0; i < n; i++) CIM_WDMA_DATA = chunks[i];
    CIM_WDMA_CTRL = 0x00;
}
...
static void cim_load_bias_at(const uint32_t *bias, uint32_t n, uint32_t base_addr) {
    for (uint32_t i = 0; i < n; i++) CIM_BIAS(base_addr + i) = bias[i];
}
...
static void cim_toggle_bank(void) {
    CIM_PING_CTRL = 1;
}
...
static int cim_fusion_copy(uint32_t n_elements) {
    ...
    for (volatile uint32_t t = 0; t < 100000; t++) {
        if (CIM_FUSION_STATUS & 0x2) return 0;
    }
    return 1;
}
...
static void run_inference(uint32_t fc1_tiles, uint32_t fc1_bias_words) {
    ...
    uint32_t diag_sum = 0;
    for (int di = 0; di < 16; di++) diag_sum += (uint32_t)pixels[di];
