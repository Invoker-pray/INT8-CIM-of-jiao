...
## 1.5 Phase C: Layer Fusion — OBUF→IBUF 内部直拷 — 🟡 RTL v7 完成, 待上板验证 (2026-05-09)
...
**Regression:** PASS (3/3). **Committed:** `db99cf8`. **Bitstream:** `bitstream&hwh/checkpoint11/`. WNS=-0.899, WHS=0.025, LUT 32.55%, FF 10.09%, BRAM 88.21%, DSP 100%. On-board test pending.
...
| ✅ DONE | Phase C: Layer Fusion — OBUF→IBUF 内部直拷, 消除 FC→FC DMA round-trip | RTL v7 DONE (capture byte 0 in F_WAIT_MUX, regression PASS), bitstream built (checkpoint11, WNS=-0.899), on-board test pending | 中 | 1w | 2026-05-09 |
