...
## 1.5 Phase C: Layer Fusion — OBUF→IBUF 内部直拷 — 🟡 RTL v5 完成, 待上板验证 (2026-05-08)

**目标：** 消除 FC→FC 层间 DMA round-trip（read_output S2MM → DDR → load_input MM2S），用硬件内部 OBUF→IBUF 拷贝替代。

**方法：**
- 新增 4 状态 FSM (F_IDLE → F_RD_FIRST → F_PACK → F_WRITE) 在 `cim_axi_lite_slave.sv`
...
- N 字节拷贝耗时 N+1 周期 + ~6% tile 边界开销 (每 16 字节额外 1 拍)
...
- **Bitstream v5: 构建中 (`bitstream&hwh/checkpoint8/`)**
- 上板测试: 待进行
...
| ✅ DONE | Phase C: Layer Fusion — OBUF→IBUF 内部直拷, 消除 FC→FC DMA round-trip | RTL v5 DONE (bias_addr fix, regression PASS), bitstream v5 building, on-board test pending | 中 | 1w | 2026-05-08 |
