...
- **Profiled bottleneck**: LeNet-5 DMA 模式下 ~128 ms/image。Profiler 实测分解：setup 41ms (32.0%), load_x 23ms (17.9%), im2col 21ms (16.0%), read_out S2MM 19.65ms (15.3%), dma_x_setup 8.9ms, compute 7.2ms (5.6%), dma_x_transfer 5.6ms。read_out 已从 257ms (MMIO) 降至 19.65ms (S2MM DMA, 13×)。下一步优化：setup (layer-wise 预加载) 和 load_x (批量输入预打包)。
